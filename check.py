...
for ptid, group in df.groupby("PtID"):
    ...
    days = np.arange(len(group)).reshape(-1, 1)  # numeric index for slope
...
if "Sex" in patient_df.columns:
    le = LabelEncoder()
    patient_df["Sex"] = le.fit_transform(
        patient_df["Sex"])  # Male/Female -> 0/1
...
# ============================
# Step 5: Predict for a single patient
# ============================
ptid_to_predict = patient_df["PtID"].iloc[0]  # pick first patient for demo
features = patient_df[patient_df["PtID"] == ptid_to_predict].drop(
    columns=["PtID", "Outcome"]
)
X_new = scaler.transform(features)

prob = model.predict_proba(X_new)[:, 1][0]
print(
    f"\nPatient {ptid_to_predict} - Predicted 90-day mortality probability: {prob:.3f}")
